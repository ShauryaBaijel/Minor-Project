...
class ResultClass:
	def __init__(self,root):
		...
		#====TITLE====
		title=Label(self.root,text="ADD STUDENT RESULT",font=("goudy old style",20,"bold"),bg="orange",fg="black").place(x=5,y=15,width=1185,height=50)
		#==========WIDGETS=============
		...
		btn_search=Button(self.root,text="SEARCH",font=("goudy old style",15,"bold"),bg="#4d155c",fg="white",cursor="hand2",command=self.search).place(x=500,y=100,width=100,height=28)
		...
		btn_clear = Button(self.root, text="CLEAR", font=("goudy old style", 15, "bold"), bg="lightgray",activebackground="lightgray",cursor="hand2",command=self.clear).place(x=430, y=420, width=120, height=35)
...
#===========================================
	...
	def fetch_roll(self):
		...
		cur=con.cursor()
		try:
			cur.execute("select roll number from student")
			rows=cur.fetchall()
			if len(rows)>0:
				for row in rows:
